...
from freqtrade.optimize.hyperopt.hyperopt_optimizer import HyperOptimizer, INITIAL_POINTS
...
class Hyperopt:
    ...
    def duplicate_optuna_asked_points(self, trial: Trial, asked_trials: list[Trial]) -> bool:
        asked_trials_no_dups = []
    ...
    def get_asked_points(self, n_points: int, dimensions: dict) -> tuple[list[Any], list[bool]]:
        ...
        asked_non_tried += [
            x for x in optuna_asked_trials if not self.duplicate_optuna_asked_points(x, optuna_asked_trials)
        ]
        i = 0
        ...
        if len(asked_non_tried) < n_points:
            logger.warning(f"duplicate params detected. Please check if search space is not too small!")
            self.count_skipped_epochs += n_points -len(asked_non_tried)

        # logger.info(f"asked_non_tried: {[x.params for x in asked_non_tried]}")
    ...
    def start(self) -> None:
        ...
        if self.count_skipped_epochs > 0:
            logger.info(
                ...
                f"skipped due to duplicate parameters. count_retry_ask: {self.count_retry_ask}"
            )
